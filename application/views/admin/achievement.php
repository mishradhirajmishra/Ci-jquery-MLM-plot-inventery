            ...
            <tr>
             <td><?php echo $col["user_id"];?></td>
              <td ><?php echo $col['user_name'];?></td>
                <td ><?php  $x=$this->admin_model->list_user_role_by_id($col['user_role']); echo $x['name'];?></td>

                <?php $d_bus=$this->admin_model->direct_business($col["user_id"]); ?>
                <?php $under_ass=$this->admin_model->all_under_associate_by_sponser_id($col["user_id"]); ?>
                <?php $ind=0;foreach($under_ass as $row){ $i_bus=$this->admin_model->direct_business($row['user_id']); $ind += $i_bus;}?>
             <td><?php echo $d_bus; ?> + <?php echo $ind; ?> = <?php $total=$ind+$d_bus; echo $total ?>

             </td>
                <?php $used=$this->admin_model->reward_sum_by_sponser_id($col["user_id"]);  $rem=$total-$used;?>
