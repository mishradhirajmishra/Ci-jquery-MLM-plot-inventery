      ...
      <?php if($col['user_role']!="admin") {?>
            <tr>
             <td><?php echo $col["user_id"];?></td>
                ...
                <!--======================================================-->
                <?php
                $customer=$this->admin_model->all_booking_by_sponser_id( $col['user_id']);
                $d_commission=0; foreach ($customer as $row){
                    $x=$this->admin_model->associate_commission_m( $row['id'],$row['sponser_id']);
                    $d_commission += $x;
                }
                $i_commission= 0; $under_associate=$this->admin_model->all_under_associate_by_sponser_id( $col['user_id']);
                $com=0;  foreach ($under_associate as $row){
                    $x = $this->admin_model-> total_indirect_commission($row['user_id'],$row['user_role'],$col['user_role']);
                    $i_commission += $x;
                }
             ...
             <td><?php echo $total; ?></td>
                <?php  $z= $this->admin_model->total_commission_associate_id($col['user_id']) ?>
             <td><?php echo $z; ?></td>
