...
<table id="example" >
    ...
    <?php foreach ($booking as $col) { ?>
            ...
            <tr>
                ...
                <td style="width: 12%"><?php $x=$this->user_model->list_plot_byid($col['plot_id']); echo $x['plot_no'];?> </td>
                <td><?php echo $col['user_name'];?></td>
                <?php $id=$col['id']; $mo=$col['user_phone']; $name=$col['user_name'];  ?>
                <td style="width: 12%"><?php  $amt=$this->user_model->all_instalment_by_date($id,$month,$year);if($amt==0){ echo '<span class="label label-sm btn-red">Not Paid</span>';}else{ echo '<span class="label label-sm btn-green">Paid</span>';}; ?></td>
