...
<div class="col-md-12">
            <!--=====================================-->
            <label class="col-xs-1">Sellect:</label>
            <div class="col-xs-2">
...
<table id="example" >
    ...
    <?php foreach ($booking as $col) { ?>
            ...
            <tr>
                <td><?php $x=$this->user_model->list_sector_byid($col['sector']); echo $x['sec_name'];?> </td>
                <td><?php $x=$this->user_model->list_plot_byid($col['plot_id']); echo $x['plot_no'];?> </td>
                <td><?php echo $col['user_name'];?></td>
                <td><?php  $amt=$this->user_model->booking_amt($col['id']); $ins=round(($col['total_price']-$amt)/$col['emi_time']); echo $ins; ?></td>
               <?php $id=$col['id']; $mo=$col['user_phone']; $name=$col['user_name'];  ?>
                <td><a id="pay<?php echo $id; ?>" class="gold-bt" onclick='pay(<?php echo $id; ?>,<?php echo $mo; ?>,"<?php echo $name; ?>")'>Send</a></td>
