...
class User_model extends CI_Model {
    ...
    function all_user_by_sponser_id(){
        $data= array('sponser_id'=>$_SESSION['user_id']);
    ...
    function all_user_date($year,$month){
        $data= array('month <='=>$month,'year'=>$year,'sponser_id'=>$_SESSION['user_id']);
        $data2= array('month <='=>$month,'year'=>$year,'user_id'=>$_SESSION['user_id']);
        $this->db->where($data);
    ...
    function find_sponser($id){
        if($id==$_SESSION['user_id']){
            ...
            $data = array('user_id' => $id, 'sponser_id' => $_SESSION['user_id']);
        }
        $x= $this->db->get_where("users",$data)->row_array();
        return $x;
    ...
    function yearly_sale($year){
       /* $year= date('Y');*/
        $data= array('year' =>$year,'sponser_id'=>$_SESSION['user_id']);
        $this->db->select_sum('stalment');
    ...
    function monthaly_sale($year,$month){
       /* $month= date('m');*/
        $data= array('year' =>$year,'month' =>$month,'sponser_id'=>$_SESSION['user_id']);
        $this->db->select('date');
        $this->db->select_sum('stalment');
    ...
    function all_booking(){
        $data= array('sponser_id'=>$_SESSION['user_id']);
        $query = $this->db->get_where('book_plot',$data);
        return $query->result_array();
    }
...
/*++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
    ...
    function direct_commission($id){
        $customer=$this->user_model->all_booking_by_sponser_id( $id);
    ...
    function bdm_commission($id){
        ...
        $under_associate=$this->user_model->all_under_associate_by_sponser_id_and_his_role( $id,8);
        $total_associate_commission=0;
        foreach ($under_associate as $row) {
            $total_associate_commission += $this->user_model->associate_commission($row['user_id']);
        }
        $total_associate_commission = ($total_associate_commission * 2)/3;
    ...
    function asm_commission($id){
        ...
        $under_bdm=$this->user_model->all_under_associate_by_sponser_id_and_his_role( $id,7);
        $total_bdm_commission = 0;
        foreach ( $under_bdm as $row) {
            $total_bdm_commission += $this->user_model->bdm_commission($row['user_id']);
        }
    ...
    function ap_commission($id){
        ...
        $under_asm=$this->user_model->all_under_associate_by_sponser_id_and_his_role( $id,6);
        $total_asm_commission = 0;
        foreach ( $under_asm as $row) {
            $total_asm_commission += $this->user_model->asm_commission($row['user_id']);
        }
    ...
    function bp_commission($id){
        ...
        $under_ap=$this->user_model->all_under_associate_by_sponser_id_and_his_role( $id,5);
        $total_ap_commission = 0;
        foreach ( $under_ap as $row) {
            $total_ap_commission += $this->user_model->ap_commission($row['user_id']);
        }
    ...
    function vp_commission($id){
        ...
        $under_bp=$this->user_model->all_under_associate_by_sponser_id_and_his_role( $id,4);
        $total_bp_commission = 0;
        foreach ( $under_bp as $row) {
            $total_bp_commission += $this->user_model->bp_commission($row['user_id']);
        }
    ...
    function dd_commission($id){
        ...
        $direct_comm=$this->direct_commission($id);
        /*=============Indirect Commission From Bp==============*/
        $under_vp=$this->user_model->all_under_associate_by_sponser_id_and_his_role( $id,3);
        $total_vp_commission = 0;
        foreach ( $under_vp as $row) {
            $total_vp_commission += $this->user_model->vp_commission($row['user_id']);
        }
    ...
    function director_commission($id){
        ...
        $under_dd=$this->user_model->all_under_associate_by_sponser_id_and_his_role( $id,3);
        $total_dd_commission = 0;
        foreach ( $under_dd as $row) {
           $total_dd_commission += $this->user_model->dd_commission($row['user_id']);
        }
    ...
    function total_commission($id,$role){
        $com=0;
        if( $role==1){$com = $this->user_model->director_commission($id);}
        if( $role==2){$com = $this->user_model->dd_commission($id);}
        if( $role==3){$com = $this->user_model->vp_commission($id);}
        if( $role==4){$com = $this->user_model->bp_commission($id);}
        if( $role==5){$com = $this->user_model->ap_commission($id);}
        if( $role==6){$com = $this->user_model->asm_commission($id);}
        if( $role==7){$com = $this->user_model->bdm_commission($id);}
        if( $role==8){$com = $this->user_model->associate_commission($id);}
        return round($com);
    }
    function total_indirect_commission($user_id,$user_role,$sponser_role){
        $com=0;
       
        if( $sponser_role==1 && $user_role==2){$com = $this->user_model->total_commission($user_id,$user_role); $com= ($com * 1)/19;}
        if( $sponser_role==1 && $user_role==3){$com = $this->user_model->total_commission($user_id,$user_role); $com= ($com * 1)/9;}
        if( $sponser_role==1 && $user_role==4){$com = $this->user_model->total_commission($user_id,$user_role); $com= ($com * 2)/8;}
        if( $sponser_role==1 && $user_role==5){$com = $this->user_model->total_commission($user_id,$user_role); $com= ($com * 3)/7;}
        if( $sponser_role==1 && $user_role==6){$com = $this->user_model->total_commission($user_id,$user_role); $com= ($com * 2)/3;}
        if( $sponser_role==1 && $user_role==7){$com = $this->user_model->total_commission($user_id,$user_role); $com= ($com * 1);}
        if( $sponser_role==1 && $user_role==8){$com = $this->user_model->total_commission($user_id,$user_role); $com= ($com * 7)/3;}

        if( $sponser_role==2 && $user_role==3){$com = $this->user_model->total_commission($user_id,$user_role); $com= ($com * 1)/18;}
        if( $sponser_role==2 && $user_role==4){$com = $this->user_model->total_commission($user_id,$user_role); $com= ($com * 3)/16;}
        if( $sponser_role==2 && $user_role==5){$com = $this->user_model->total_commission($user_id,$user_role); $com= ($com * 5)/14;}
        if( $sponser_role==2 && $user_role==6){$com = $this->user_model->total_commission($user_id,$user_role); $com= ($com * 7)/12;}
        if( $sponser_role==2 && $user_role==7){$com = $this->user_model->total_commission($user_id,$user_role); $com= ($com * 9)/10;}
        if( $sponser_role==2 && $user_role==8){$com = $this->user_model->total_commission($user_id,$user_role); $com= ($com * 13)/6;}

        if( $sponser_role==3 && $user_role==4){$com = $this->user_model->total_commission($user_id,$user_role); $com= ($com * 1)/8;}
        if( $sponser_role==3 && $user_role==5){$com = $this->user_model->total_commission($user_id,$user_role); $com= ($com * 2)/7;}
        if( $sponser_role==3 && $user_role==6){$com = $this->user_model->total_commission($user_id,$user_role); $com= ($com * 1)/2;}
        if( $sponser_role==3 && $user_role==7){$com = $this->user_model->total_commission($user_id,$user_role); $com= ($com * 4)/5;}
        if( $sponser_role==3 && $user_role==8){$com = $this->user_model->total_commission($user_id,$user_role); $com= ($com * 2);}

        if( $sponser_role==4 && $user_role==5){$com = $this->user_model->total_commission($user_id,$user_role); $com= ($com * 1)/7;}
        if( $sponser_role==4 && $user_role==6){$com = $this->user_model->total_commission($user_id,$user_role); $com= ($com * 1)/3;}
        if( $sponser_role==4 && $user_role==7){$com = $this->user_model->total_commission($user_id,$user_role); $com= ($com * 3)/5;}
        if( $sponser_role==4 && $user_role==8){$com = $this->user_model->total_commission($user_id,$user_role); $com= ($com * 5)/3;}


        if( $sponser_role==5 && $user_role==6){$com = $this->user_model->total_commission($user_id,$user_role); $com= ($com * 1)/6;}
        if( $sponser_role==5 && $user_role==7){$com = $this->user_model->total_commission($user_id,$user_role); $com= ($com * 2)/5;}
        if( $sponser_role==5 && $user_role==8){$com = $this->user_model->total_commission($user_id,$user_role); $com= ($com * 4)/3;}

        if( $sponser_role==6 && $user_role==7){$com = $this->user_model->total_commission($user_id,$user_role); $com= ($com * 1)/5;}
        if( $sponser_role==6 && $user_role==8){$com = $this->user_model->total_commission($user_id,$user_role); $com= ($com * 1) ;}

        if( $sponser_role==7 && $user_role==8){$com = $this->user_model->total_commission($user_id,$user_role); $com= ($com * 2)/6;}  
