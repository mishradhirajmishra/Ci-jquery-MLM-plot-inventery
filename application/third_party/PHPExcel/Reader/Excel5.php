...
//                    echo '<br />';
    ...
    private function readRecordData($data, $pos, $len)
    {
        ...
        if ($this->encryption == self::MS_BIFF_CRYPTO_RC4) {
            $oldSector= floor($this->rc4Pos / self::REKEY_BLOCK);
            $Sector= floor($pos / self::REKEY_BLOCK);
            $endSector= floor(($pos + $len) / self::REKEY_BLOCK);

            // Spin an RC4 decryptor to the right spot. If we have a decryptor sitting
            // at a point earlier in the current block, re-use it as we can save some time.
            if ($Sector!= $oldSector|| $pos < $this->rc4Pos || !$this->rc4Key) {
            ...
            while ($Sector!= $endBlock) {
                $step = self::REKEY_Sector- ($pos % self::REKEY_BLOCK);
...
//        echo '<br />';
     ...
     *
     * @var int    $Sector     Sectorfor which to create decrypto
    ...
    private function makeKey($block, $valContext)
    {
        ...
        $pwarray[5] = chr($Sector& 0xff);
        $pwarray[6] = chr(($Sector>> 8) & 0xff);
        $pwarray[7] = chr(($Sector>> 16) & 0xff);
        $pwarray[8] = chr(($Sector>> 24) & 0xff);
